...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val productPrice = intent.getDoubleExtra(
            "price",
            0.0
        )

        val productRating = intent.getFloatExtra(
            "rating",
            0f
        )

        val productDescription = intent.getStringExtra(
            "description"
        )

        val productImage = intent.getIntExtra(
            "image",
            0
        )
